      ...
      <el-col :span="16">
        ...
        <el-card class="profile-card" shadow="hover">
          ...
          <el-form v-if="!editingBasic" label-width="100px">
            ...
            <el-form-item label="邮箱">{{ userStore.user?.email }}</el-form-item>
            <el-form-item label="手机">{{ userStore.user?.phone || '未设置' }}</el-form-item>
            <el-form-item label="角色">
              <el-tag :type="roleTagType">{{ roleLabel }}</el-tag>
            </el-form-item>
            <el-form-item label="注册时间">{{ formatDate(userStore.user?.created_at) }}</el-form-item>
          </el-form>

          <el-form v-else ref="basicFormRef" :model="basicForm" :rules="basicRules" label-width="100px">
        ...
        <el-card v-if="isPatient" class="profile-card" shadow="hover">
          <template #header>
            <div class="card-header">
              <span>🏥 健康档案</span>
              <el-button type="primary" size="small" @click="startEditPatient">编辑</el-button>
            </div>
          </template>
          ...
          <el-form v-if="!editingPatient" label-width="110px">
            <el-row :gutter="16">
              <el-col :span="12">
                <el-form-item label="姓名">{{ patientProfile?.real_name || '未设置' }}</el-form-item>
                <el-form-item label="年龄">{{ patientProfile?.age ?? '未设置' }}</el-form-item>
                <el-form-item label="血型">{{ patientProfile?.blood_type || '未设置' }}</el-form-item>
                <el-form-item label="身高">{{ patientProfile?.height_cm ? patientProfile.height_cm + ' cm' : '未设置' }}</el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="性别">{{ patientProfile?.gender || '未设置' }}</el-form-item>
                <el-form-item label="出生日期">{{ patientProfile?.birth_date ? formatDate(patientProfile.birth_date) : '未设置' }}</el-form-item>
                <el-form-item label="体重">{{ patientProfile?.weight_kg ? patientProfile.weight_kg + ' kg' : '未设置' }}</el-form-item>
                <el-form-item label="科室">{{ patientProfile?.department || '未设置' }}</el-form-item>
              </el-col>
            </el-row>
            <el-form-item label="过敏史">{{ patientProfile?.allergies || '无' }}</el-form-item>
            <el-form-item label="紧急联系人">{{ patientProfile?.emergency_contact || '未设置' }}</el-form-item>
            <el-form-item label="紧急电话">{{ patientProfile?.emergency_phone || '未设置' }}</el-form-item>
          </el-form>

          <el-form v-else ref="patientFormRef" :model="patientForm" label-width="110px">
            <el-row :gutter="16">
              <el-col :span="12">
                <el-form-item label="姓名"><el-input v-model="patientForm.real_name" /></el-form-item>
                ...
                <el-form-item label="身高(cm)"><el-input-number v-model="patientForm.height_cm" :min="0" :max="300" :precision="1" /></el-form-item>
              </el-col>
              <el-col :span="12">
                ...
                <el-form-item label="出生日期">
                  <el-date-picker v-model="patientForm.birth_date" type="date" value-format="YYYY-MM-DD" />
                </el-form-item>
                <el-form-item label="体重(kg)"><el-input-number v-model="patientForm.weight_kg" :min="0" :max="500" :precision="1" /></el-form-item>
              </el-col>
            </el-row>
            <el-form-item label="过敏史"><el-input v-model="patientForm.allergies" type="textarea" /></el-form-item>
            <el-form-item label="紧急联系人"><el-input v-model="patientForm.emergency_contact" /></el-form-item>
            <el-form-item label="紧急电话"><el-input v-model="patientForm.emergency_phone" /></el-form-item>
            <el-form-item>
              <el-button type="primary" :loading="savingPatient" @click="savePatient">保存</el-button>
      ...
      <el-col :span="8">
        ...
        <template v-if="isPatient">
          <el-card class="stats-side-card" shadow="hover">
            ...
            <div class="stat-row">
              <span class="stat-label">检测次数</span>
              <span class="stat-value">{{ patientStats.total_detections }}</span>
            </div>
          </el-card>
        </template>
      </el-col>
...
import { updateProfileApi, changePasswordApi, getPatientProfileApi, updatePatientProfileApi, getProfileStatsApi } from '@/api/auth'
...
const basicRules = {
  username: [{ required: true, message: '请输入用户名', trigger: 'blur' }, { min: 3, max: 50 }],
...
async function saveBasic() {
  ...
  try {
    await updateProfileApi({ username: basicForm.username, email: basicForm.email, phone: basicForm.phone })
...
const pwdRules = {
  ...
  new_password: [{ required: true, min: 6, max: 100, message: '密码长度6-100位', trigger: 'blur' }],
  confirm_password: [{ required: true, validator: validateConfirmPwd, trigger: 'blur' }],
}
...
const patientForm = reactive({
  real_name: '', age: null, gender: '', birth_date: null,
  blood_type: '', height_cm: null, weight_kg: null,
  allergies: '', emergency_contact: '', emergency_phone: '',
})

function startEditPatient() {
  const p = patientProfile.value
  if (p) Object.assign(patientForm, {
    real_name: p.real_name || '', age: p.age, gender: p.gender || '',
    birth_date: p.birth_date, blood_type: p.blood_type || '',
...
function cancelEditPatient() { editingPatient.value = false }
...
const doctorStats = reactive({ patient_count: 0, total_detections: 0, total_records: 0 })
...
onMounted(async () => {
  if (isPatient.value) {
    try { patientProfile.value = await getPatientProfileApi() } catch (e) { /* */ }
  }
  loadStats()
})
</script>
...
.stats-side-card { margin-bottom: 16px; }
.stat-row { display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f0f0f0; }
.stat-row:last-child { border-bottom: none; }
.stat-row .stat-label { color: #909399; }
.stat-row .stat-value { font-size: 20px; font-weight: bold; color: #409eff; }
</style>
