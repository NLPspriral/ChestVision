...
class RemoteTrainingCommandMixin:
    ...
    def _build_user_command(self, task: TrainingTask) -> str:
        ...
        return (
            ...
            f"        'lr': get_lr(trainer),\n"
            f"    }})\n"
            f"def clear_single_process_distributed_env():\n"
            f"    keys = ('RANK', 'LOCAL_RANK', 'WORLD_SIZE', 'MASTER_ADDR', 'MASTER_PORT', 'GROUP_RANK', 'ROLE_RANK', 'LOCAL_WORLD_SIZE', 'NODE_RANK')\n"
            f"    existing = {{key: os.environ.get(key) for key in keys if os.environ.get(key) is not None}}\n"
            f"    try:\n"
            f"        world_size = int(os.environ.get('WORLD_SIZE') or '1')\n"
            f"    except ValueError:\n"
            f"        world_size = 1\n"
            f"    if world_size <= 1:\n"
            f"        for key in keys:\n"
            f"            os.environ.pop(key, None)\n"
            f"        if existing:\n"
            f"            print('single-process training: cleared distributed env vars ' + json.dumps(existing, ensure_ascii=False), flush=True)\n"
            f"    else:\n"
            f"        print('distributed training env preserved: ' + json.dumps(existing, ensure_ascii=False), flush=True)\n"
            f"try:\n"
            ...
            f"    clear_single_process_distributed_env()\n"
            f"    from ultralytics import YOLO\n"
            f"    model = YOLO(model_name)\n"
            f"    model.add_callback('on_train_epoch_end', on_train_epoch_end)\n"
            f"    model.train(data=data_yaml, epochs=epochs, imgsz=img_size, batch=batch_size, optimizer=optimizer, lr0=lr0, project=output_dir, name='run', exist_ok=True, verbose=True, save=True, plots=False, device='0', amp=False)\n"
            f"except Exception as exc:\n"
