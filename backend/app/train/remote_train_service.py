...
class RemoteTrainingService:
    ...
    def _build_user_command(self, task: TrainingTask) -> str:
        ...
        return (
            ...
            f"python - <<'PY'\n"
            f"import json, os, time, zipfile\n"
            f"mount_dir = {dataset!r}\n"
            f"output_dir = {output!r}\n"
            f"work_dir = '/tmp/remote_train_dataset'\n"
            f"data_yaml = os.path.join(mount_dir, 'data.yaml')\n"
            f"if not os.path.exists(data_yaml):\n"
            f"    zip_name = os.environ.get('RAW_DATASET_FILENAME', 'dataset.zip')\n"
            f"    zip_path = os.path.join(mount_dir, zip_name)\n"
            f"    if not os.path.exists(zip_path):\n"
            ...
            f"    candidates = []\n"
            f"    for root, _, files in os.walk(work_dir):\n"
            f"        if 'data.yaml' in files:\n"
            f"            candidates.append(os.path.join(root, 'data.yaml'))\n"
            f"    if not candidates:\n"
            f"        raise FileNotFoundError('data.yaml 不存在')\n"
            f"    data_yaml = sorted(candidates, key=len)[0]\n"
            f"report = {{\n"
            f"    'ok': True,\n"
            ...
            f"open('{output}/_SUCCESS','w').write(json.dumps(payload))\n"
            f"PY"
        )
